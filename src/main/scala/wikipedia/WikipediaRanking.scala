...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.textFile(WikipediaData.filePath).map(str => WikipediaData.parse(str))
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.map{ case (lang: String, list: Iterable[WikipediaArticle]) => lang -> list.size }
      .collect()
      .toList
